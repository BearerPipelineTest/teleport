...
Initialize and configure Linux ACLs for certificate destination directory. Used
to lock down access to certificates on host when running long lived Machine ID
service.
...
| `--owner`            | Defines Linux user:group owner of `--destination-dir`. Defaults to the bot user if unspecified.       |
| `--bot-user`         | Enables Linux ACLs and defines Linux user that can write short-lived certificates to `--destination-dir`.  |
| `--reader-user`      | Enables Linux ACLs and defines Linux user that will read short-lived certificates from `--destination-dir`. |
